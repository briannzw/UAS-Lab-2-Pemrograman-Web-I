<?php
    include("aksi_modify_news.php");
?>

<html>
    <head>
        <title>GrandChase Modify News</title>
    ...
    <body>
        ...
        <form action="modify_news.php<?= ("?id=".$_GET['id']); ?>" method="post" autocomplete="off" enctype="multipart/form-data">
            <div class="container rounded pb-4 px-4 mt-5" style="border: 1px solid lightgray; border-radius: 5px; width: 65%;">
                ...
                <div class="row">
                    <div class="col">
                        <p>Image</p>
                        <p style="font-size: 12px;">Old image</p>
                        <img src="<?= $image_folder.$image ?>" class="rounded img-fluid mb-3" alt="News Image" style="max-width: 400px;">
                        <input type="hidden" name="old-content-img" value="<?= $image ?>"></input>
                        <input type="file" name="content-img" class="form-control" accept="image/*"></input>
                    </div>
                </div>
                <div class="row align-items-center mt-3 mb-2">
                    <div class="col">
                        <p>Topic</p>
                        <input type="text" name="topic" class="form-control" value="<?= isset($_POST["topic"]) ? htmlentities($_POST["topic"]) : $topic; ?>" required></input>
                    </div>
                </div>
                <div class="row">
                    <div class="col">
                        <p>Content</p>
                        <textarea name="content" style="width: 100%; max-width: 100%; margin-bottom: 10px;" class="form-control" required><?= isset($_POST["content"]) ? htmlentities($_POST["content"]) : $content; ?></textarea>
                    </div>
                </div>
                <input type="hidden" name="date" value="<?= date("M j\, Y") ?>"></input>
                <input class="btn btn-outline-primary mt-5" name="modify_news" type="submit"></input>
            </div>
